...
class UnfoldNet(nn.Module):
    ...
    def forward(
        ...
        """
        Returns: X, U, theta  [, traj dict if return_traj=True]
        Traj contains lists of (pmui, radar_mse, obj) per stage.
        """
        cfg = self.cfg
        ...
        # ── Initialise ────────────────────────────────────────────────────────
        L = H_br.shape[-2]  # works for (L,N) and (B,L,N)
        if is_batch:
            B = H_bu.shape[0]
        ...
        if is_batch and X.dim() == 2:
            # init_XU returns unbatched if Rd is 2D; broadcast
        ...
        for t in range(self.T):
            ...
            if return_traj:
                with torch.no_grad():
                    ...
                    err  = H_e @ X - S
                    p    = float((err.abs() ** 2).sum(dim=(-2,-1)).mean().item())
                    r    = float(((X - U).abs() ** 2).sum(dim=(-2,-1)).mean().item())
                    o    = cfg.rho * p + (1.0 - cfg.rho) * r
                    traj["pmui"].append(p)
                    traj["radar"].append(r)
                    traj["obj"].append(o)
