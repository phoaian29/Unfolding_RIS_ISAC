...
Phase parameterisation: θ = exp(jφ)
  ...
  φ ← φ - α · dphi
  θ = exp(jφ)  [unit-modulus by construction, no division NaN]
"""
...
class ThetaLayer(nn.Module):
    ...
    def _build_Gd(H_bu, H_ru, H_br, X, S):
        """
        B = H_ru^H H_ru, C = H_br X X^H H_br^H, D = H_br X T^H H_ru
